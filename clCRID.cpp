		...
		case 0x43524944://CRID
		{
			if (info.dataType == 1) {
				...
				for (unsigned int i = 0, count = _utf.GetPageCount(); i < count; i++) {
					...
					case 0x40534656: // @SFV
						if (is_demux_video) {
							sprintf_s(savename, _countof(savename), "%s\\video_%s", directory, fix_filename);
					...
					case 0x40534641: // @SFA
						if (is_demux_audio) {
							sprintf_s(savename, _countof(savename), "%s\\audio_%s", directory, fix_filename);
							char ext[4];
...
void clCRID::ClearMaskAudio() {
	memset(_audioMask, 0, 0x20);
}
